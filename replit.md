...
- `investor-presentation.html` - Premium investor presentation (cover + 15 slides + Thank You, incl. Financial Model slides 12-15 rendered via financial-model.js): full-viewport slide sections, animated gold line art cover, portfolio cards w/ logos, corporate structure diagram, business model flow, advantages/growth/why lists, roadmap track, final slide w/ contacts + data room link + verbatim legal disclaimer. Print CSS renders one 16:9 landscape page per slide.
...
- `admin.html` - Password-protected admin panel (noindex). Login posts to /api/admin/login (ADMIN_ACCESS_CODE secret — separate from data room code; same token/rate-limit pattern). Editor for Financial Model data: KPI cards (1-12, value+label) and capital allocation categories (2-10, must total exactly 100% — live total indicator). Saves via PUT /api/admin/financial-model (Bearer token, sessionStorage 12h). adm-* styles in presentation.css.
- `financial-model.js` - Shared renderer for the Financial Model & Capital Allocation data on investor-relations.html and investor-presentation.html. Fetches GET /api/financial-model, renders into data-fm-kpis/donut/legend/bars/calc hooks (KPI cards, SVG donut, allocation bars, illustrative calculator with slider+input). GOLD_SHADES palette; sets html[data-fm-ready].
...
## Financial Model & Capital Allocation (editable)
- Data lives in Postgres (`financial_model` table, single JSONB row id=1, seeded with FINANCIAL_MODEL_DEFAULTS in server.js): `kpis` (label/value) + `allocation` (name/pct, sum=100).
- Public read: GET /api/financial-model. Admin write: POST /api/admin/login (ADMIN_ACCESS_CODE secret) → PUT /api/admin/financial-model (validates lengths + pct sum=100).
- Rendered by financial-model.js on: investor-relations.html `#ir-financial-model` section (8 sub-blocks: fm-platform KPIs, fm-allocation donut+bars, fm-scenarios 3 illustrative scenarios, fm-value flow, philosophy quote, fm-risk, fm-why 6 cards, fm-calculator interactive tool + verbatim disclaimer; fm-* CSS in styles.css) AND investor-presentation.html slides 12-15 (fm-pres-* CSS in presentation.css incl. print overrides; calculator excluded from PDF, pointer text instead).
- Edited via admin.html. KPIs/allocation changes appear on both pages automatically; **regenerate the PDF after data changes** so slides 12-13 stay current.
- Compliance: every figure labeled illustrative/hypothetical; no projections, returns, valuations, or offers anywhere in the section.

## Recent Changes
- 2026-07-08: **Financial Model & Capital Allocation feature** — New admin-editable data layer (Postgres `financial_model` JSONB row, GET /api/financial-model, POST /api/admin/login + PUT /api/admin/financial-model with ADMIN_ACCESS_CODE secret, validation pct sum=100, rate limiting + timing-safe compare). Shared financial-model.js renderer (KPI cards, SVG donut, allocation bars, illustrative calculator). New `#ir-financial-model` section on investor-relations.html (8 sub-blocks incl. 3 illustrative scenarios, value-creation flow, risk mgmt, 6 why-capital cards, interactive calculator + verbatim disclaimer; fm-* CSS in styles.css). Presentation gained slides 12-15 (fm-pres-* CSS + print grid overrides in presentation.css); PDF regenerated to 17 pages. New admin.html panel (login → edit KPIs/allocation → save; adm-* CSS). All figures labeled illustrative/hypothetical per compliance rules.
